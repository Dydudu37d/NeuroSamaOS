...
_Bool ATAReadSector48(u8 Channel, u8 Drive, u64 LBA, u8* Buffer) {
    ...
    outb(io_base + ATA_REG_SECTOR_CNT, 1 >> 8);
    ...
    return !(ATAReadStatus(io_base) & ATA_STATUS_ERR);
}
...
_Bool ATADetectDrive(u8 Channel, u8 Drive) {
    ...
    if (buffer[83] & (1 << 10)) {
        ...
        ATADrives[Channel][Drive].MaxLBA28 = 
            ...
            0x0FFFFFFF : ATADrives[Channel][Drive].TotalSectors;
    }
...
_Bool ATAReadSector(u8 Channel, u8 Drive, u64 LBA, u8* Buffer) {
    ...
    if (ATADrives[Channel][Drive].SupportsLBA48 || LBA > 0x0FFFFFFF) {
